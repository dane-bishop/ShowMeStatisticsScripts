...
def upsert_player_volleyball_gamelog(conn, player_id: int, rows: Iterable[Dict[str, Any]]):
    ...
    with conn, conn.cursor() as cur:
        for g in rows:
            ...
            cur.execute("""
                INSERT INTO player_game_volleyball (
                ...
                DO UPDATE SET
                    source_game_id = COALESCE(EXCLUDED.source_game_id, player_game_volleyball.source_game_id),

                    wl = COALESCE(EXCLUDED.wl, player_game_volleyball.wl),
                    sp = COALESCE(EXCLUDED.sp, player_game_volleyball.sp),
                    
                    k = COALESCE(EXCLUDED.k, player_game_volleyball.k),
                    ae = COALESCE(EXCLUDED.ae, player_game_volleyball.ae),
                    ta = COALESCE(EXCLUDED.ta, player_game_volleyball.ta),
                    h_pct = COALESCE(EXCLUDED.h_pct, player_game_volleyball.h_pct),
                    
                    ast = COALESCE(EXCLUDED.ast, player_game_volleyball.ast),
                    e = COALESCE(EXCLUDED.e, player_game_volleyball.e),
                    
                    sa = COALESCE(EXCLUDED.sa, player_game_volleyball.sa),
                    se = COALESCE(EXCLUDED.se, player_game_volleyball.se),

                    dre = COALESCE(EXCLUDED.dre, player_game_volleyball.dre),
                    dd = COALESCE(EXCLUDED.dd, player_game_volleyball.dd),
                    
                    solo = COALESCE(EXCLUDED.solo, player_game_volleyball.solo),
                    blk_ast = COALESCE(EXCLUDED.blk_ast, player_game_volleyball.blk_ast),   
                    blk_e = COALESCE(EXCLUDED.blk_e, player_game_volleyball.blk_e),
                    tot_blk = COALESCE(EXCLUDED.tot_blk, player_game_volleyball.tot_blk),
                    
                    bhe = COALESCE(EXCLUDED.bhe, player_game_volleyball.bhe),
                    pts = COALESCE(EXCLUDED.pts, player_game_volleyball.pts)
            """, (
                player_id, game_id, sgid,
                g.get("wl"), g.get("sp"), 
                g.get("k"), g.get("ae"), g.get("ta"), g.get("h_pct"),
                g.get("ast"), g.get("e"),
                g.get("sa"), g.get("se"), 
                g.get("dre"), g.get("dd"),
                g.get("solo"), g.get("blk_ast"), g.get("blk_e"), g.get("tot_blk"),
                g.get("bhe"), g.get("pts"),
            ))
...
def upsert_player_volleyball_season_highs(conn, player_id: int, highs):
